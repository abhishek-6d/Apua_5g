...
public class RewardPointsCalculationDAO {
	...
	public void calculateRewardPointsStep1(GenericDTO genericDTO) {
		...
		try {
			...
			logFile = "" + calculationDTO.getTransactionId() + "|" + dateFormat.format(new Date()) + "|" + calculationDTO.getSubscriberNumber() + "|" + calculationDTO.getRewardPointsCategory() + "|" + calculationDTO.getVolume() + "|";
			...
			if (commonUtil.isItChar(calculationDTO.getSubscriberNumber())) {
				...
				if (tabDTO != null) {
					...
					if (!calculationDTO.isStatusPoints() && tabDTO.getStatusID() != 1)
						throw new CommonException("ADSL Number : " + calculationDTO.getSubscriberNumber() + " is not in active state");
				...
				SubscriberNumberTabDTO subscriberNumberTabDTO = tableDetailsDAO.getSubscriberNumberDetails(Long.parseLong(calculationDTO.getSubscriberNumber()));
				...
				if (subscriberNumberTabDTO != null) {
					if (!calculationDTO.isStatusPoints() && subscriberNumberTabDTO.getStatusID() != 1) {
						throw new CommonException(Cache.getServiceStatusMap().get("REW_SUB_INACTIVE_" + calculationDTO.getDefaultLanguage()).getStatusCode(), commonUtil.getStatusDescription(
								Cache.getServiceStatusMap().get("REW_SUB_INACTIVE_" + calculationDTO.getDefaultLanguage()).getStatusDesc(), new String[] { NotificationTokens.subscriberNumber }, new String[] { calculationDTO.getSubscriberNumber() }));
					}
				...
				if (calculationDTO.getLoyaltyID() == null) {
					AccountNumberTabDTO accountNumberTabDTO = tableDetailsDAO.getAccountNumberDetails(calculationDTO.getSubscriberNumber()+"");
					if (accountNumberTabDTO != null) {

						if (!calculationDTO.isStatusPoints() && accountNumberTabDTO.getStatusID() != 1)
							throw new CommonException(Cache.getServiceStatusMap().get("REW_ACCNO_INACTIVE_" + calculationDTO.getDefaultLanguage()).getStatusCode(), commonUtil.getStatusDescription(
									Cache.getServiceStatusMap().get("REW_ACCNO_INACTIVE_" + calculationDTO.getDefaultLanguage()).getStatusDesc(), new String[] { NotificationTokens.accountNumber },
									new String[] { calculationDTO.getSubscriberNumber() }));

						calculationDTO.setLoyaltyID(accountNumberTabDTO.getLoyaltyID());

						List<LoyaltyRegisteredNumberTabDTO> list = (List<LoyaltyRegisteredNumberTabDTO>) tableDetailsDAO.getLoyaltyRegisteredNumberDetails(accountNumberTabDTO.getLoyaltyID(), accountNumberTabDTO.getAccountNumber());
				...
				if (calculationDTO.getLoyaltyID() == null) {
					loyaltyProfileTabDTO = tableDetailsDAO.getLoyaltyProfile(Long.parseLong(calculationDTO.getSubscriberNumber()));
					if (loyaltyProfileTabDTO != null) {

						if (!calculationDTO.isStatusPoints() && loyaltyProfileTabDTO.getStatusID() != 1)
							throw new CommonException(Cache.getServiceStatusMap().get("REW_LOYALTYID_INACTIVE_" + calculationDTO.getDefaultLanguage()).getStatusCode(), commonUtil.getStatusDescription(
									Cache.getServiceStatusMap().get("REW_LOYALTYID_INACTIVE_" + calculationDTO.getDefaultLanguage()).getStatusDesc(), new String[] { NotificationTokens.loyaltyID },
									new String[] { calculationDTO.getSubscriberNumber() }));
			...
			if (calculationDTO.getLoyaltyID() == null) {
				...
				logger.info(calculationDTO.getTransactionId() + ": Loyalty ID not exist for Subsriber Number :" + calculationDTO.getSubscriberNumber());
				throw new CommonException(Cache.getServiceStatusMap().get("REW_SUB_INVALID_" + calculationDTO.getDefaultLanguage()).getStatusCode(), commonUtil.getStatusDescription(
						Cache.getServiceStatusMap().get("REW_SUB_INVALID_" + calculationDTO.getDefaultLanguage()).getStatusDesc(), new String[] { NotificationTokens.subscriberNumber }, new String[] { calculationDTO.getSubscriberNumber() }));
			...
			logFile += calculationDTO.getLoyaltyID() + "|" + calculationDTO.getCalculatedRewardPoints() + "|" + calculationDTO.getCalculatedStatusPoints() + "|" + calculationDTO.getProfileTierID() + "|" + calculationDTO.getNextTierID() + "|"
					+ Cache.getServiceStatusMap().get("REW_SUCCESS_" + calculationDTO.getDefaultLanguage()).getStatusCode() + "|'" + Cache.getServiceStatusMap().get("REW_SUCCESS_" + calculationDTO.getDefaultLanguage()).getStatusDesc() + "'|||"
			...
			if (e instanceof CommonException) {
				...
				logFile += calculationDTO.getLoyaltyID() + "|" + calculationDTO.getCalculatedRewardPoints() + "|" + calculationDTO.getCalculatedStatusPoints() + "|" + calculationDTO.getProfileTierID() + "|" + calculationDTO.getNextTierID() + "|"
						+ exception.getStatusCode() + "|'" + e.getLocalizedMessage() + "'|||" + (calculationDTO.getSubscriberType() == null ? "" : calculationDTO.getSubscriberType());
			} else {
				/*genericDTO.setStatusCode(Cache.getServiceStatusMap().get("REW_FAIL_" + calculationDTO.getDefaultLanguage()).getStatusCode());
				genericDTO.setStatus(Cache.getServiceStatusMap().get("REW_FAIL_" + calculationDTO.getDefaultLanguage()).getStatusDesc());*/
				try {
				...
				logFile += calculationDTO.getLoyaltyID() + "|" + calculationDTO.getCalculatedRewardPoints() + "|" + calculationDTO.getCalculatedStatusPoints() + "|" + calculationDTO.getProfileTierID() + "|" + calculationDTO.getNextTierID() + "|"
						+ Cache.getServiceStatusMap().get("REW_FAIL_" + calculationDTO.getDefaultLanguage()).getStatusCode() + "|'" + e.getLocalizedMessage() + "'|||"
						+ (calculationDTO.getSubscriberType() == null ? "" : calculationDTO.getSubscriberType());
			}
		} finally {
	...
	public void calculateRewardPointsStep2(GenericDTO genericDTO, Object object) throws CommonException {
		...
		String   exp=null;
		long threshold = Long.parseLong(Cache.getConfigParameterMap().get("REWARD_POINTS_THRESHOLD") != null ? Cache.getConfigParameterMap().get("REWARD_POINTS_THRESHOLD").getParameterValue() : "-1");
		...
		try {
			...
			while (loopCounter < 10) {
				...
				loyaltyTransactionMap.put(LoyalityTransactionConstants.reqTransactionID, calculationDTO.getTransactionId());//added S
	    		loyaltyTransactionMap.put(LoyalityTransactionConstants.volume,String.valueOf(calculationDTO.getOriginalVolume()));//added S

				if (calculationDTO.isSubscriber()) {
					if (loopCounter == 0)
						subscriberNumberTabDTO = (SubscriberNumberTabDTO) object;
					else
						subscriberNumberTabDTO = tableDetailsDAO.getSubscriberNumberDetails(Long.parseLong(calculationDTO.getSubscriberNumber()));
					loyaltyTransactionMap.put(LoyalityTransactionConstants.subscriberNumber,String.valueOf(subscriberNumberTabDTO.getSubscriberNumber()));//added S
					loyaltyTransactionMap.put(LoyalityTransactionConstants.accountNumber,String.valueOf(subscriberNumberTabDTO.getAccountNumber()));//added S
					//loyaltyTransactionTabDTO.setSubscriberNumber("" + subscriberNumberTabDTO.getSubscriberNumber());
					logger.info(calculationDTO.getTransactionId() + ": Subscriber Number : " + calculationDTO.getSubscriberNumber() + " Points : " + subscriberNumberTabDTO.getPoints());
				} else if (calculationDTO.isAccount()) {
					if (loopCounter == 0)
						accountNumberTabDTO = (AccountNumberTabDTO) object;
					else
						accountNumberTabDTO = tableDetailsDAO.getAccountNumberDetails(calculationDTO.getSubscriberNumber());

					logger.info(calculationDTO.getTransactionId() + ": Account Number : " + calculationDTO.getSubscriberNumber() + " Points : " + accountNumberTabDTO.getPoints());
					...
					loyaltyTransactionMap.put(LoyalityTransactionConstants.subscriberNumber,tabDTO.getADSLNumber());//added S
					//loyaltyTransactionTabDTO.setSubscriberNumber(tabDTO.getADSLNumber());
					logger.info(calculationDTO.getTransactionId() + ": ADSL Number : " + calculationDTO.getSubscriberNumber() + " Points : " + tabDTO.getPoints());
				} else {
				...
				if (loyaltyProfileTabDTO == null) {
					logger.info(calculationDTO.getTransactionId() + ": Loyalty ID exist Loyalty ID : " + calculationDTO.getLoyaltyID() + ", but record not exist in table ");
					throw new CommonException(Cache.getServiceStatusMap().get("REW_SUB_INVALID_" + calculationDTO.getDefaultLanguage()).getStatusCode(), commonUtil.getStatusDescription(
							Cache.getServiceStatusMap().get("REW_SUB_INVALID_" + calculationDTO.getDefaultLanguage()).getStatusDesc(), new String[] { NotificationTokens.subscriberNumber }, new String[] { calculationDTO.getSubscriberNumber() }));
				}
				...
				if (!calculationDTO.isStatusPoints() && loyaltyProfileTabDTO.getStatusID() != 1) {
					logger.info(calculationDTO.getTransactionId() + ": Loyalty ID : " + loyaltyProfileTabDTO.getLoyaltyID() + "  Status : " + Cache.getStatusMap().get(loyaltyProfileTabDTO.getStatusID()));
					// throw new
					// CommonException("Loyalty ID : "+loyaltyProfileTabDTO.getLoyaltyID()+"  Status : "+Cache.getStatusMap().get(loyaltyProfileTabDTO.getStatusID()));
					throw new CommonException(Cache.getServiceStatusMap().get("REW_LOYALTYID_INACTIVE_" + calculationDTO.getDefaultLanguage()).getStatusCode(), commonUtil.getStatusDescription(
							Cache.getServiceStatusMap().get("REW_LOYALTYID_INACTIVE_" + calculationDTO.getDefaultLanguage()).getStatusDesc(), new String[] { NotificationTokens.loyaltyID }, new String[] { "" + loyaltyProfileTabDTO.getLoyaltyID() }));
				...
				logger.info(calculationDTO.getTransactionId() + ": Before, LoyaltyID Number : " + loyaltyProfileTabDTO.getLoyaltyID() + " Points : " + loyaltyProfileTabDTO.getRewardPoints() + " Tier ID " + loyaltyProfileTabDTO.getTierId()
						+ " Last Reward Date " + profiledate + " DayWiseRewardPoints " + loyaltyProfileTabDTO.getDayWiseRewardPoints());
				...
				logger.info(calculationDTO.getTransactionId() + ": After, LoyaltyID Number : " + loyaltyProfileTabDTO.getLoyaltyID() + " Points : " + loyaltyProfileTabDTO.getRewardPoints() + " Tier ID " + loyaltyProfileTabDTO.getTierId()
						+ " Last Reward Date " + profiledate + " DayWiseRewardPoints " + calculationDTO.getDayWiseRewardPoints());
				...
				if (threshold > -1) {
					if (calculationDTO.getDayWiseRewardPoints() > threshold) {
						logger.info(calculationDTO.getTransactionId() + " : Loyalty ID : " + calculationDTO.getLoyaltyID() + " , Total Reward Points " + calculationDTO.getDayWiseRewardPoints()
						...
						throw new CommonException("SC2000", "Reward Points has cross the configured Threshold value :" + threshold);
					}
				}

				if (!calculationDTO.isStatusPoints())
					loyaltyTransactionMap.put(LoyalityTransactionConstants.curRewardPoints,String.valueOf(loyaltyProfileTabDTO.getRewardPoints()));//added S
					//loyaltyTransactionTabDTO.setCurRewardPoints(loyaltyProfileTabDTO.getRewardPoints());

					loyaltyTransactionMap.put(LoyalityTransactionConstants.curStatusPoints,String.valueOf(loyaltyProfileTabDTO.getStatusPoints()));//added S
				...
				if (loyaltyTransactionTabDTO.getCurRewardPoints() == null || loyaltyTransactionTabDTO.getCurRewardPoints() == 0)
					loyaltyTransactionMap.put(LoyalityTransactionConstants.curRewardPoints,String.valueOf(loyaltyTransactionTabDTO.getPreRewardPoints()));//added S
					//loyaltyTransactionTabDTO.setCurRewardPoints(loyaltyTransactionTabDTO.getPreRewardPoints());

				if (loyaltyTransactionTabDTO.getCurStatusPoints() == null || loyaltyTransactionTabDTO.getCurStatusPoints() == 0)
					loyaltyTransactionMap.put(LoyalityTransactionConstants.curStatusPoints,String.valueOf(loyaltyTransactionTabDTO.getPreStatusPoints()));//added S
		...
		/*		loyaltyTransactionTabDTO.setRewardPoints(rewardPoints);
				...
				loyaltyTransactionMap.put(LoyalityTransactionConstants.curRewardPoints,String.valueOf(loyaltyProfileTabDTO.getRewardPoints()));//added S
				loyaltyTransactionMap.put(LoyalityTransactionConstants.preRewardPoints,String.valueOf(currentRewardPoints));//added S
				loyaltyTransactionMap.put(LoyalityTransactionConstants.rewardPoints,String.valueOf(calculationDTO.getRequestRewardPoints()));//added S
				loyaltyTransactionMap.put(LoyalityTransactionConstants.reqTransactionID,String.valueOf(calculationDTO.getTransactionId()));//added S
				loyaltyTransactionMap.put(LoyalityTransactionConstants.statusPoints,String.valueOf(loyaltyProfileTabDTO.getStatusPoints()));//added S
				loyaltyTransactionMap.put(LoyalityTransactionConstants.channel,String.valueOf(Cache.channelDescDetails.get(calculationDTO.getChannel())));//added S
				loyaltyTransactionMap.put(LoyalityTransactionConstants.preTierId,String.valueOf(calculationDTO.getProfileTierID()));//added S
				loyaltyTransactionMap.put(LoyalityTransactionConstants.curTierId,String.valueOf(calculationDTO.getNextTierID()));//added S
				loyaltyTransactionMap.put(LoyalityTransactionConstants.serverId,Cache.cacheMap.get("SERVER_ID"));//added S
				loyaltyTransactionMap.put(LoyalityTransactionConstants.statusDescription,Cache.getConfigParameterMap().get("MANUAL_POINT_ADJUSTMENT_TXN_STATUS_DESC").getParameterValue());
				...
				if (!calculationDTO.isStatusPoints()) {
					...
					if (calculationDTO.isSubscriber()) {
						...
						String sql = " UPDATE  " + infoDAO.getSubscriberNumberDBTable(calculationDTO.getSubscriberNumber()) + " SET POINTS=POINTS+?,COUNTER=?,POINTS_UPDATED_DATE=? " + " WHERE SUBSCRIBER_NUMBER=? AND COUNTER=?";
						query = session.createSQLQuery(sql).setParameter(0, rewardPoints).setParameter(1, (subscriberNumberTabDTO.getCounter() >= Integer.MAX_VALUE ? 1 : subscriberNumberTabDTO.getCounter() + 1))
						.setParameter(2, new Date()).setParameter(3, subscriberNumberTabDTO.getSubscriberNumber()).setParameter(4, subscriberNumberTabDTO.getCounter());

						System.out.println("get updatioh SQL --------------- >" + (System.currentTimeMillis() - start));

					} else if (calculationDTO.isAccount()) {
						String sql = " UPDATE " + infoDAO.getAccountNumberDBTable(calculationDTO.getSubscriberNumber()) + " SET POINTS=POINTS+?,COUNTER=? " + " WHERE ACCOUNT_NO=? AND COUNTER=? ";
						query = session.createSQLQuery(sql).setParameter(0, rewardPoints).setParameter(1, (accountNumberTabDTO.getCounter() >= Integer.MAX_VALUE ? 1 : accountNumberTabDTO.getCounter() + 1))
								.setParameter(2, accountNumberTabDTO.getAccountNumber()).setParameter(3, accountNumberTabDTO.getCounter());

					} else if (calculationDTO.isADSL()) {
						String sql = " UPDATE " + infoDAO.getADSLNumberDBTable(calculationDTO.getSubscriberNumber()) + " SET POINTS=POINTS+?,COUNTER=? " + " WHERE ADSL_NO=? AND COUNTER=?";

						query = session.createSQLQuery(sql).setParameter(0, rewardPoints).setParameter(1, (tabDTO.getCounter() >= Integer.MAX_VALUE ? 1 : tabDTO.getCounter() + 1)).setParameter(2, tabDTO.getADSLNumber())
								.setParameter(3, tabDTO.getCounter());
				...
				String sql = " UPDATE " + infoDAO.getLoyaltyProfileDBTable(calculationDTO.getLoyaltyID() + "") + " SET REWARD_POINTS=REWARD_POINTS+?,TIER_POINTS=TIER_POINTS+? ,STATUS_POINTS=STATUS_POINTS+?,"
						+ " TIER_ID=?,COUNTER=?,TIER_UPDATED_DATE=?,POINTS_CREDITED_DATE=?,DAYWISE_REWARD_POINTS=? " + " WHERE LOYALTY_ID=? AND COUNTER=?";
				int index = 0;
				Query query1 = session.createSQLQuery(sql).
						setParameter(index++, rewardPoints).
						setParameter(index++, rewardPoints).
						setParameter(index++, statusPoints).
						setParameter(index++, loyaltyProfileTabDTO.getTierId())
						.setParameter(index++, (loyaltyProfileTabDTO.getCounter() >= Integer.MAX_VALUE ? 1 : loyaltyProfileTabDTO.getCounter() + 1)).
						setParameter(index++, loyaltyProfileTabDTO.getTierUpdatedDate()).setParameter(index++, new Date())
						.setParameter(index++, calculationDTO.getDayWiseRewardPoints()).setParameter(index++, loyaltyProfileTabDTO.getLoyaltyID()).setParameter(index++, loyaltyProfileTabDTO.getCounter());
				...
				if (i > 0 && j > 0) {
					...
					loyaltyTransactionTabDTO.setStatusID(calculationDTO.isStatusPoints() ? LoyaltyTransactionStatus.statusPointsOnlyAdded : LoyaltyTransactionStatus.rewardsPointsAdded);
					loyaltyTransactionTabDTO.setTestNumber(calculationDTO.getTestNumber()); // Test
																							// Number
																							// flag
*/
					...
					loyaltyTransactionMap.put(LoyalityTransactionConstants.statusId,String.valueOf(calculationDTO.isStatusPoints() ? LoyaltyTransactionStatus.statusPointsOnlyAdded : LoyaltyTransactionStatus.manuallyRewardsPointAdded));//added S
					loyaltyTransactionMap.put(LoyalityTransactionConstants.testNumber,String.valueOf(calculationDTO.getTestNumber()));//added S
					...
					tierAndBonusCalculationDAO = new TierAndBonusCalculationDAO();
					  ...
					  configParmeterDTO = Cache.configParameterMap.get("TIER_POINTS_VALIDITY_DAYS");
						...
						if(tierAndBonusPointDetailsDTO!=null){
								...
								tierAndBonusPointDetailsDTO.setTierPoints(tierAndBonusPointDetailsDTO.getTierPoints()+rewardPoints);
			...
			try {
				if (calculationDTO.getProfileTierID() < calculationDTO.getNextTierID()) {
					...
					logger.info(calculationDTO.getTransactionId() + ": Tier Got Upgraded for Loyalty ID:  " + calculationDTO.getLoyaltyID() + " From " + calculationDTO.getProfileTierID() + " to " + calculationDTO.getNextTierID());
					if (loyaltyProfileTabDTO.getContactNumber() != null && !loyaltyProfileTabDTO.getContactNumber().trim().equals("")) {
						languageDAO = new LanguageDAO();
						loyaltyProfileTabDTO.setDefaultLanguage(languageDAO.getLanguageID(loyaltyProfileTabDTO.getContactNumber()));
						//NotificationPanel notificationPanel = new NotificationPanel();
						//notificationPanel.rewardPointsTierUpgradeNotification(loyaltyProfileTabDTO, calculationDTO);
						...
						if (!isCodeSent) {
							logger.info("TransactionId:" + calculationDTO.getTransactionId() + "Code we are sending for first time");
							ApplicationCodesDTO applicationCodesDTO = getCodeForCurrentTier(calculationDTO.getTransactionId(), calculationDTO.getLoyaltyID(), calculationDTO.getNextTierID(), "101");
							if (applicationCodesDTO != null) {
								// Map in table msisdn and app
								auditAppMsisdnMapping(calculationDTO.getTransactionId(), loyaltyProfileTabDTO, applicationCodesDTO, loyaltyProfileTabDTO.getContactNumber());
								//notificationPanel.tierUpdgradeAppInfoNotification(loyaltyProfileTabDTO, calculationDTO, applicationCodesDTO);
	...
	public void auditAppMsisdnMapping(String txnId, LoyaltyProfileTabDTO loyaltyProfileTabDTO, ApplicationCodesDTO applicationCodesDTO, String msisdn) {
		TierUpgradationDAO tierUpgradationDAO = null;
		...
		try {
			...
			do {
				...
				if (applicationCodeList == null || applicationCodeList.size() == 0) {
					logger.info("Service : PointCalculation -- Transaction ID :" + txnId + " LoyaltyId " + loyaltyId + " There is no code" + " mapped/remaining for this tier " + nextTierID);
					/*
					 * Since there is no codes remaining, we are assuming that
					 * code is blocked and loop can be exit
					 */
					isCodeBlocked = true;
				} else {
					applicationCodesDTO = applicationCodeList.get(0);
					appCode = applicationCodesDTO.getAppCode();
					logger.debug("Service : PointCalculation -- Transaction ID :" + txnId + " LoyaltyId :" + loyaltyId + " Fetched code :" + appCode + " for Tier :" + nextTierID);
			...
			logger.error("Service : PointCalculation -- Transaction ID :" + txnId + " LoyaltyId :" + loyaltyId + " " + "Exception in selecting appCodes ");
		} finally {
			tierUpgradationDAO = null;
	...
	public void pointsCalculation(LoyaltyProfileTabDTO loyaltyProfileTabDTO, RewardPointsCalculationDTO calculationDTO) throws CommonException {
		...
		if ((calculationDTO.getRequestRewardPoints() != null && calculationDTO.getRequestRewardPoints() > 0) || (calculationDTO.getRequestStatusPoints() != null && calculationDTO.getRequestStatusPoints() > 0)) {
			...
			logger.info("Calculated reward points" + calculationDTO.getRequestRewardPoints());
			calculationDTO.setCalculatedRewardPoints(calculationDTO.getRequestRewardPoints() == null ? 0 : calculationDTO.getRequestRewardPoints());
			calculationDTO.setCalculatedStatusPoints(calculationDTO.getRequestStatusPoints() == null ? 0 : calculationDTO.getRequestStatusPoints());
		...
		boolean isUnitCalculation = Cache.rewardPointsCategoryMap.get(new Integer(calculationDTO.getRewardPointsCategory())).getUnitsCalculation().equalsIgnoreCase("N") ? false : true;
		...
		if (calculationDTO.getVolume() == null || isUnitCalculation == false) {
			...
			List<CategoryDetailsDTO> list = Cache.tierCategoryDetailsMap.get(loyaltyProfileTabDTO.getTierId()).get(new Integer(calculationDTO.getRewardPointsCategory()));
			...
			logger.info(calculationDTO.getTransactionId() + ": Tier ID :" + loyaltyProfileTabDTO.getTierId() + "  Category ID :" + calculationDTO.getRewardPointsCategory() + "   Details :" + list);

			for (CategoryDetailsDTO detailsDTO : list) {
				if (calculationDTO.getVolume() != null && (detailsDTO.getMinValue() != null || detailsDTO.getMaxValue() != null)) {
					if (checkBetween(calculationDTO.getVolume(), detailsDTO.getMinValue(), detailsDTO.getMaxValue())) {
						loyaltyProfileTabDTO.setRewardPoints(loyaltyProfileTabDTO.getRewardPoints() + detailsDTO.getRewardPoints());
						loyaltyProfileTabDTO.setStatusPoints(loyaltyProfileTabDTO.getStatusPoints() + detailsDTO.getStatusPoints());
		...
		if (isUnitCalculation) {
			while (calculationDTO.getVolume() > 0) {
				...
				logger.info(calculationDTO.getTransactionId() + ": Tier ID :" + loyaltyProfileTabDTO.getTierId() + "  Category ID :" + calculationDTO.getRewardPointsCategory() + "   Volume : " + calculationDTO.getVolume());
					...
					if (detailsDTO.getMinValue() != null && detailsDTO.getMinValue() > calculationDTO.getVolume()) {
						...
						if (checkBetween(calculationDTO.getVolume(), detailsDTO.getMinValue(), detailsDTO.getMaxValue())) {
							...
							if (tierInfoDTO.getMaxValue() != null) {
								...
								calculateRequiredVolume(tierInfoDTO.getMaxValue(), loyaltyProfileTabDTO, detailsDTO, calculationDTO);
	...
	public void calculateRequiredVolume(Integer tierMaxValue, LoyaltyProfileTabDTO loyaltyProfileTabDTO, CategoryDetailsDTO detailsDTO, RewardPointsCalculationDTO calculationDTO) {
		Double requiredVolume = Math.ceil((tierMaxValue - loyaltyProfileTabDTO.getStatusPoints()) / detailsDTO.getStatusPoints());
		if (requiredVolume < 0) {
			loyaltyProfileTabDTO.setRewardPoints(loyaltyProfileTabDTO.getRewardPoints() + (calculationDTO.getVolume() * detailsDTO.getRewardPoints()));
			loyaltyProfileTabDTO.setStatusPoints(loyaltyProfileTabDTO.getStatusPoints() + (calculationDTO.getVolume() * detailsDTO.getStatusPoints()));

			logger.info(calculationDTO.getTransactionId() + ": Required Volume required Volume  " + calculationDTO.getVolume() + " Total Reward Points " + loyaltyProfileTabDTO.getRewardPoints() + "   Status Points "
					+ loyaltyProfileTabDTO.getStatusPoints());

			calculationDTO.setCalculatedRewardPoints(calculationDTO.getCalculatedRewardPoints() + ((calculationDTO.getVolume() * detailsDTO.getRewardPoints())));
			calculationDTO.setCalculatedStatusPoints(calculationDTO.getCalculatedStatusPoints() + ((calculationDTO.getVolume() * detailsDTO.getStatusPoints())));

			logger.info(calculationDTO.getTransactionId() + ": Required Volume Tier Before " + loyaltyProfileTabDTO.getTierId());
			checkTierUpgration(loyaltyProfileTabDTO, calculationDTO);
			logger.info(calculationDTO.getTransactionId() + ": Required Volume Tier After " + loyaltyProfileTabDTO.getTierId());
		...
		loyaltyProfileTabDTO.setRewardPoints(loyaltyProfileTabDTO.getRewardPoints() + (requiredVolume * detailsDTO.getRewardPoints()));
		loyaltyProfileTabDTO.setStatusPoints(loyaltyProfileTabDTO.getStatusPoints() + (requiredVolume * detailsDTO.getStatusPoints()));

		logger.info(calculationDTO.getTransactionId() + ": Required Volume required Volume  " + requiredVolume + " Total Reward Points " + loyaltyProfileTabDTO.getRewardPoints() + "   Status Points " + loyaltyProfileTabDTO.getStatusPoints());

		calculationDTO.setCalculatedRewardPoints(calculationDTO.getCalculatedRewardPoints() + ((requiredVolume * detailsDTO.getRewardPoints())));
		calculationDTO.setCalculatedStatusPoints(calculationDTO.getCalculatedStatusPoints() + ((requiredVolume * detailsDTO.getStatusPoints())));
	...
	public void calculateFullVolume(LoyaltyProfileTabDTO loyaltyProfileTabDTO, CategoryDetailsDTO detailsDTO, RewardPointsCalculationDTO calculationDTO) {
		loyaltyProfileTabDTO.setRewardPoints(loyaltyProfileTabDTO.getRewardPoints() + (calculationDTO.getVolume() * detailsDTO.getRewardPoints()));
		loyaltyProfileTabDTO.setStatusPoints(loyaltyProfileTabDTO.getStatusPoints() + (calculationDTO.getVolume() * detailsDTO.getStatusPoints()));
		logger.info(calculationDTO.getTransactionId() + ": Full Volume Total Reward Points " + loyaltyProfileTabDTO.getRewardPoints() + "  Total Status Points " + loyaltyProfileTabDTO.getStatusPoints());

		calculationDTO.setCalculatedRewardPoints(calculationDTO.getCalculatedRewardPoints() + ((calculationDTO.getVolume() * detailsDTO.getRewardPoints())));
		calculationDTO.setCalculatedStatusPoints(calculationDTO.getCalculatedStatusPoints() + ((calculationDTO.getVolume() * detailsDTO.getStatusPoints())));

		logger.info(calculationDTO.getTransactionId() + ": Full Volume Tier Before " + loyaltyProfileTabDTO.getTierId());
		checkTierUpgration(loyaltyProfileTabDTO, calculationDTO);
	...
	public boolean checkBetween(Double volume, Double minValue, Double maxValue) {
		...
		return volume != null ? (minValue != null ? minValue <= volume : true) && (maxValue != null ? volume <= maxValue : true) : false;

	}// checkBetween

	public void checkTierUpgration(LoyaltyProfileTabDTO loyaltyProfileTabDTO, RewardPointsCalculationDTO calculationDTO) {
		...
		while (iterator.hasNext()) {
			...
			if ((infoDTO.getMinValue() != null ? infoDTO.getMinValue() <= statusPoints : true) && (infoDTO.getMaxValue() != null ? statusPoints < infoDTO.getMaxValue() : true)) {
				logger.info(calculationDTO.getTransactionId() + ": Got Tier For this Status Points :" + statusPoints + " Min Value :" + infoDTO.getMinValue() + "  Max Value :" + infoDTO.getMaxValue() + "   Tier ID : " + infoDTO.getTierId());
				if(infoDTO.getTierId()>loyaltyProfileTabDTO.getTierId()){// Check for not to downgrade the tier
					logger.info(calculationDTO.getTransactionId()+": Tier is going to upgrade for LoyaltyID "+loyaltyProfileTabDTO.getLoyaltyID()+" from  "+loyaltyProfileTabDTO.getTierId()+" to   "+infoDTO.getTierId());
					loyaltyProfileTabDTO.setTierId(infoDTO.getTierId());
				}else
					logger.info(calculationDTO.getTransactionId()+": Tier remains same for LoyaltyID  "+loyaltyProfileTabDTO.getLoyaltyID());
			}
		...
		if (!loyaltyProfileTabDTO.getTierId().equals(calculationDTO.getProfileTierID())) {
			...
			try{
				tableDetailsDAO=new TableDetailsDAO();
				logger.info(calculationDTO.getProfileTierID() + "########Got Tier ID After Tire Change is###########" + loyaltyProfileTabDTO.getTierId());
				if (loyaltyProfileTabDTO.getTierId() == 4) {
					DateFormat df = new SimpleDateFormat("ddMMyyyyHHmmss");
					LoyaltyRegisteredNumberTabDTO loyaltyRegisteredNumberTabDTO = null;
					

					loyaltyRegisteredNumberTabDTO = tableDetailsDAO.getLoyaltyRegisteredNumberDetails(loyaltyProfileTabDTO.getLoyaltyID(), loyaltyProfileTabDTO.getContactNumber());
					
					String callType = Cache.getCacheMap().get("CALL_TYPE_" + loyaltyRegisteredNumberTabDTO.getAccountTypeId()) != null ? Cache.getCacheMap().get("CALL_TYPE_" + loyaltyRegisteredNumberTabDTO.getAccountTypeId()) : "1";
					...
					if((loyaltyRegisteredNumberTabDTO.getAccountTypeId()==200 || loyaltyRegisteredNumberTabDTO.getAccountTypeId()==201))
							if(calculationDTO.getSubscriberNumber()!=null)
							{						
	...
	public int checkTierUpgrationBasedOnDate(Double statusPoints, LoyaltyProfileTabDTO loyaltyprofile) {
		...
		while (iterator.hasNext()) {
			...
			if ((infoDTO.getMinValue() != null ? infoDTO.getMinValue() <= statusPoints : true) && (infoDTO.getMaxValue() != null ? statusPoints < infoDTO.getMaxValue() : true)) {
				logger.info(" Status Points :" + statusPoints + " Min Value :" + infoDTO.getMinValue() + "  Max Value :" + infoDTO.getMaxValue() + "   Tier ID : " + infoDTO.getTierId());
	...
	public Double[] pointsCalculation(int tierId, RewardPointsCalculationDTO calculationDTO) throws CommonException {
		...
		List<CategoryDetailsDTO> list = Cache.tierCategoryDetailsMap.get(new Integer(tierId)).get(new Integer(calculationDTO.getRewardPointsCategory()));

		if ((list == null || list.size() == 0) && calculationDTO.getRequestRewardPoints() == null && calculationDTO.getRequestStatusPoints() == null) {
			logger.info(calculationDTO.getTransactionId() + ": Tier Category Details Not defind");
			throw new CommonException("Tier Category Details Not defind");
		}

		logger.info(calculationDTO.getTransactionId() + ": Tier ID :" + tierId + "  Category ID :" + calculationDTO.getRewardPointsCategory() + "   Details :" + list);
		...
		for (CategoryDetailsDTO detailsDTO : list) {
			...
			if (calculationDTO.getVolume() != null && (detailsDTO.getMinValue() != null || detailsDTO.getMaxValue() != null)) {
				if (checkBetween(calculationDTO.getVolume(), detailsDTO.getMinValue(), detailsDTO.getMaxValue())) {
					...
					points[0] = isUnitCalculation ? calculationDTO.getVolume() * detailsDTO.getRewardPoints() : detailsDTO.getRewardPoints();
					points[1] = isUnitCalculation ? calculationDTO.getVolume() * detailsDTO.getStatusPoints() : detailsDTO.getStatusPoints();
					break;
				}
			} else {
		...
		if (points[0] == null || points[1] == null) {
			logger.info(calculationDTO.getTransactionId() + ": Request Volume is not macthing with configuration Volume :" + calculationDTO.getVolume());
			throw new CommonException("Request Volume is not macthing with configuration Volume");
		}
		return points;
	}// pointsCalculation

	public void insertLoyaltyThresholdDetails(LoyaltyProfileTabDTO profileTabDTO, RewardPointsCalculationDTO calculationDTO, long threshold, double rewardPoints) {
		Session session = null;
		Transaction transaction = null;
		String sql = null;
		Query query = null;
		try {
			...
			sql = "insert into LOYALTY_THRESHOLD_DETAILS (ID,LOYALTY_ID,SUBSCRIBER_NUMBER,ACCOUNT_NUMBER,VOLUME,REQ_REWARD_POINTS,PONITS_GIVEN,THRESHOLD,EARN_POINTS)" + "values (LOYALTY_THRESHOLD_DETAILS_SEQ.nextVal,?,?,?,?,?,?,?,?)";
			query = session.createSQLQuery(sql).setParameter(0, profileTabDTO.getLoyaltyID()).setParameter(1, calculationDTO.isSubscriber() ? calculationDTO.getSubscriberNumber() : null)
					.setParameter(2, calculationDTO.isAccount() ? calculationDTO.getSubscriberNumber() : null).setParameter(3, calculationDTO.getOriginalVolume() == null ? 0.0 : calculationDTO.getOriginalVolume())
					.setParameter(4, calculationDTO.getRequestRewardPoints() == null ? 0.0 : calculationDTO.getRequestRewardPoints()).setParameter(5, calculationDTO.getDayWiseRewardPoints() == null ? 0.0 : calculationDTO.getDayWiseRewardPoints())
	...
	private String getRequestXML(RewardPointsCalculationDTO rewardPointsCalculationDTO) {
		...
		sb.append("<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns:arb=\"http://omantel.om/ArborService\">");
		...
		if (rewardPointsCalculationDTO.getSubscriberNumber().length() == 11)
			sb.append("<CustMobileNo>" + rewardPointsCalculationDTO.getSubscriberNumber().substring(3) + "</CustMobileNo>");
		else
	...
	public boolean insertIntoPrepaidInstantTable(RewardPointsCalculationDTO rewardPointsCalculationDTO) {
		...
		try {
			...
			logger.info("Class => RewardPointsCalculationDAO :: PAymnt Agency ID"+rewardPointsCalculationDTO.getPaymentAgencyID());
			...
			postPaidInstantDTO.setActualPaymentTimeStamp(dateFormat2.parse(rewardPointsCalculationDTO.getActualPaymentDate())+"");
	...
	public GenericDTO settingStatus(String key,RewardPointsCalculationDTO calculationDTO,GenericDTO genericDTO) throws CommonException
	{
		ServiceStatusDTO actionServiceDetailsDTO = null;
		try
		{
			logger.info(">>>>>>>>>>>>>>>>Language id"+calculationDTO.getDefaultLanguage()+">>>>>>>>>>>>>jkey"+key);
		if((actionServiceDetailsDTO = Cache.getServiceStatusMap().get(key+"_"+calculationDTO.getDefaultLanguage().trim()))!=null){
			genericDTO.setStatusCode(actionServiceDetailsDTO.getStatusCode());
			genericDTO.setStatus(actionServiceDetailsDTO.getStatusDesc());
		}else
		{
			logger.warn(" Service : -- Transaction ID : "+calculationDTO.getTransactionId()+" MoNumber "+calculationDTO.getSubscriberNumber()+""+ " No Key Defined in Service Status table for Key :- "+key);
	...
	public CDRInformationDTO buildingCdrDtoAndWritingCdr(RewardPointsCalculationDTO calculationDTO , GenericDTO genericDTO)
	{
	...
	/*public boolean insertDayWiseEntry()
	{
		Date expiryDate = null;
		Calendar c =null;
		ConfigureParameterDTO configParmeterDTO = null;
