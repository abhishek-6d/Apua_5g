...
/*
 *Author Abhey Gupta
 *Changes for Time Interval based Rollling
 */
...
public class DailyRollingFileAppender extends FileAppender {
	...
	private long startFile = System.currentTimeMillis();

	private String fileNamePattern = "applogs";
	/**
	...
	public DailyRollingFileAppender() {
		...
		System.out.println("date pattern : " + datePattern + " || time interval : " + timeInterval + "  || fieName Pattern : " + fileNamePattern);
		LogLog.debug("STARTING TIMER");
		startTimer();
	}
	...
	private class MonitorTime extends TimerTask {
		...
		public void run() {
			while (true) {
				if (System.currentTimeMillis() >= timeInterval * 60 * 1000 + startFile) {
					try {
						LogLog.debug("Roll over called at : " + new Date());
	...
	public void activateOptions() {
		...
		if (datePattern != null && fileName != null) {
			...
			rc.setInterval(this.timeInterval);
			int type = computeCheckPeriod();
			printPeriodicity(type);
			rc.setType(type);
			File file = new File(fileName);
			this.startFile = System.currentTimeMillis();
			Date intervalDate = new Date(file.lastModified());
			Calendar calendar = Calendar.getInstance();
			calendar.setTime(intervalDate);
			int unroundedMinutes = calendar.get(Calendar.MINUTE);
			int mod = unroundedMinutes % this.timeInterval;
			calendar.add(Calendar.MINUTE, -mod);
			scheduledFilename = fileName + sdf.format(calendar.getTime());
	...
	void printPeriodicity(int type) {
		...
		case TOP_OF_MINUTE:
			if (this.timeInterval > 0) {
				LogLog.debug("Appender [" + name + "] to be rolled every" + this.timeInterval + " minute.");
			} else {
	...
	void rollOver() throws IOException {
		...
		// System.out.println("scheduled filename "+scheduledFilename);
		// System.out.println("datedFilename filename "+datedFilename);
		...
		objects.add(0, new Date());
		objects.add(1, String.valueOf(seqId));
		MessageFormat mf = new MessageFormat(fileNamePattern);
		String name = mf.format(objects.toArray());
...
class RollingCalendar extends GregorianCalendar {
	...
	public Date getNextCheckDate(Date now) {
		...
		case DailyRollingFileAppender.TOP_OF_MINUTE:
			...
			if (this.interval > 0) {
				this.add(Calendar.MINUTE, this.interval);
			} else {
