package com.sixdee.imp.request;

/**
 * 
 * @author Somesh
 * @version 1.0
 * 
 * <p>
 * <b><u>Development History</u></b><br>
 * <table border="1" width="100%">
 * <tr>
 * <td width="15%"><b>Date</b></td>
 * <td width="20%"><b>Author</b></td>
 * </tr>
 * <tr>
 * <td>April 22,2014 11:36:42 AM</td>
 * <td>Somesh</td>
 * </tr>
 * </table>
 * </p>
 */



import com.sixdee.fw.dto.GenericDTO;
import com.sixdee.fw.exception.CommonException;
import com.sixdee.fw.request.ReqAssmCommon;
import com.sixdee.imp.dto.ActDeactOfferDTO;



public class ActDeactOfferReqAssm extends ReqAssmCommon {
	
	
	
	public GenericDTO buildAssembleReq(GenericDTO genericDTO) throws CommonException {
		
		logger.info(" Class ==> ActDeactOfferReqAssm :: Method ==> buildAssembleGUIReq ");
		ActDeactOfferDTO actDeactOfferDTO = null;
		try{
			actDeactOfferDTO=(ActDeactOfferDTO)genericDTO.getObj();

		} catch (Exception e) {
			e.printStackTrace();
		} finally {
			genericDTO.setObj(actDeactOfferDTO);
			actDeactOfferDTO = null;
		}

		return genericDTO;
	}

}
