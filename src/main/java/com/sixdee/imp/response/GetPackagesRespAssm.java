...
public class GetPackagesRespAssm extends RespAssmCommon {
    /**
    ...
    public GenericDTO buildAssembleResp(GenericDTO genericDTO) throws CommonException
	{
		...
		try {
			...
			logger.info("STATUS DESC" + packageInfoDTO.getStatusDescription());
			logger.info("Getpackages Response TransactionID  "+packageInfoDTO.getTranscationId());
                        ...
                        for(Object o : all)
                        {
                            ...
                            if(packageDetails.getLanguageId()==defaultLanguage)
                                if((packageDetails.getQuantity()==null || packageDetails.getQuantity()>0)&&(packageDetails.getExpiryDate()==null || packageDetails.getExpiryDate().after(new Date())))
                                {
                                    ...
                                    p.setPackageID(String.valueOf(packageDetails.getPackageId()));
                                    ...
                                    //sajith ks sts618 ****start
                                   logger.info("adding into PackageDetailsDTO");
                                    p.setAreaLocations(packageDetails.getAreaLocations());
