...
public class InstantRewardsFrmtValidator extends FrmtValidatorCommon {
	 ...
	 */
	private Logger logger = Logger.getLogger(InstantRewardsFrmtValidator.class);
	public GenericDTO buildValidateReq(GenericDTO genericDTO) throws CommonException {
		...
		try{
			...
			instantRewardsVO = (InstantRewardsVO) reRequestHeader.getObj();
			...
			String bSubscriber = instantRewardsVO.getbSubscriberNumber();
			if(bSubscriber!=null){
				if(bSubscriber.length()==subscriberCountryCode.length()+subscriberSize){
					instantRewardsVO.setbSubscriberNumber(bSubscriber.substring(subscriberCountryCode.length()));
				}
			}
