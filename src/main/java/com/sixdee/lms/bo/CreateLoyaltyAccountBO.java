...
import com.sixdee.lms.dto.FetchProfile.Profile;
...
public class CreateLoyaltyAccountBO {
	...
	private Response generateLoyaltyAccount(String transactionId, String moNumber, String channel) throws Exception {
		...
		try {
			...
			if (!(customerDetails != null && customerDetails.get(SystemConstants.THIRDY_PARTY_RESULT_CODE) != null
					&& !customerDetails.get(SystemConstants.THIRDY_PARTY_RESULT_CODE).equalsIgnoreCase("1"))) {
			...
			if (customerDetails != null && loyaltyId != null) {
				...
				if (customerProfileTabDTOs != null) {
					...
					if ((Cache.getConfigParameterMap().get("WELCOME_POINTS_ENABLE")).getParameterValue() != null
							&& (Cache.getConfigParameterMap().get("WELCOME_POINTS_ENABLE")).getParameterValue().trim()
									.equalsIgnoreCase("Y")) {
						boolean welcomePointAddStatus = addingWelcomePointsOnRegistration(transactionId,
								System.currentTimeMillis() + "", customerDetails);
	...
	private Request getRuleRequest(String requestId, String msisdn, String keyword) {
		Date d = new Date();
	...
	private String createLoyaltyAccount(String requestId, HashMap<String, String> customerDetails, String tierId)
		...
		try {
			...
			objects = generateLoyaltyRegisteredMDNs(customerDetails, loyaltyProfileTabDTO.getLoyaltyID(),
					accountTypeId);
			...
			// logger.info("RequestId "+requestId+" Pin Created
			// "+loyaltyProfileTabDTO.getPin());
			loyaltyAccountCreationUtil = new LoyaltyAccountCreationUtil();
			isAccountCreated = loyaltyAccountCreationUtil.createAccount(requestId,
					customerDetails.get(SystemConstants.CHANNEL) != null
							? Cache.channelDetails.get(customerDetails.get(SystemConstants.CHANNEL))
							: "",
					loyaltyProfileTabDTO, loyaltyRegisteredMDNList, accountNumberList, subscriberNumberList);
			...
			cdrInformationDTO.setChannelID(customerDetails.get(SystemConstants.CHANNEL) != null
					? Cache.channelDetails.get(customerDetails.get(SystemConstants.CHANNEL))
					: "");
	...
	private int getAccountTypeId(String requestId, HashMap<String, String> customerDetails) {
		int accountTypeId = 0;
		if (customerDetails.get(SystemConstants.ACCOUNT_CATEGORY_TYPE) != null && customerDetails
				.get(SystemConstants.ACCOUNT_CATEGORY_TYPE).equalsIgnoreCase(SystemConstants.prepaidCustomerType)) {
			accountTypeId = 1;
		} else {
			accountTypeId = 2;
		}
	...
	private ArrayList<AccountNumberTabDTO> generateAccountList(HashMap<String, String> customerDetails,
		...
		try {
			...
			for (String key : customerDetails.keySet()) {
				if (key.startsWith("ACCOUNT_NUM_") || key.startsWith(SystemConstants.ACCOUNT_NUM)
						|| key.startsWith("ASA_")) {
					accountNumberTabDTO = getAccountNumberDetails(customerDetails, loyaltyID, key.startsWith("ASA"),
							key);
	...
	private AccountNumberTabDTO getAccountNumberDetails(HashMap<String, String> customerDetails, Long loyaltyID,
			boolean isASA, String key) {
		AccountNumberTabDTO accountNumberTabDTO = null;
		try {
	...
	private Object[] generateLoyaltyRegisteredMDNs(HashMap<String, String> customerDetails, Long loyaltyID,
			int accountTypeId) {
		...
		try {
			...
			loyaltyRegisteredNumberTabDTO = getLoyatltyRegisterdDetails(customerDetails, SystemConstants.ACCOUNT_NUM,
					loyaltyID, accountTypeId);
			loyaltyRegisteredNumberList.add(loyaltyRegisteredNumberTabDTO);

			subscriberNumberTabDTO = getSubscriberNumberDetails(customerDetails, SystemConstants.MSISDN, loyaltyID,
					accountTypeId);
			subscriberNumberList.add(subscriberNumberTabDTO);

			for (String key : customerDetails.keySet()) {
				if (key.startsWith("MSISDN_")) {
					loyaltyRegisteredNumberTabDTO = getLoyatltyRegisterdDetails(customerDetails, key, loyaltyID,
							accountTypeId);
					logger.info("Loyalty registered number details " + loyaltyRegisteredNumberTabDTO.getLinkedNumber()
							+ " loyaltyId " + loyaltyID + " Account number "
							+ loyaltyRegisteredNumberTabDTO.getAccountNumber());
					...
					logger.debug("Subscriber Number details " + subscriberNumberTabDTO.getSubscriberNumber()
							+ " Account Number " + subscriberNumberTabDTO.getAccountNumber());
					if (!customerDetails.get(SystemConstants.MSISDN)
	...
	private SubscriberNumberTabDTO getSubscriberNumberDetails(HashMap<String, String> customerDetails, String key,
			Long loyaltyID, int accountTypeId) {
		...
		if (!key.equals(SystemConstants.MSISDN)) {
			accountKey = customerDetails
					.get("ACCOUNT_NUM_" + customerDetails.get(key.substring(key.indexOf("_"), key.lastIndexOf("_"))));
		} else {
		...
		subscriberNumberTabDTO
				.setAccountTypeId(Integer.valueOf(customerDetails.get(SystemConstants.ACCOUNT_CATEGORY_TYPE)));
	...
	private LoyaltyRegisteredNumberTabDTO getLoyatltyRegisterdDetails(HashMap<String, String> customerDetails,
		...
		logger.info(">>>>>>>>>>KEY" + key + " >>>>>>>>>>" + customerDetails.get(key) + " >>>>>>>>>>>>>>"
				+ customerDetails.get(SystemConstants.MSISDN));
	...
	private LoyaltyProfileTabDTO generateLoyaltyProfile(String requestId, HashMap<String, String> customerDetails,
			String tierId) throws ParseException {
		...
		try {
			...
			/*
			 * Object[] obj=generateLoyaltyRegisteredMDNs(customerDetails,
			 * loyaltyProfileTabDTO.getLoyaltyID(),accountTypeId);
			 * loyaltyRegisteredNumberTabDTOs = (ArrayList<LoyaltyRegisteredNumberTabDTO>)
			 * obj[0]; accountNumberTabDTOList = generateAccountList(customerDetails,
	...
	public List<CustomerProfileTabDTO> customerProfileDtoBuilder(HashMap<String, String> customerDetails,
		...
		try {
			...
			if (customerDetails.containsKey(SystemConstants.ACCOUNT_CATEGORY_TYPE))
				customerProfileTabDTO
						.setAccountCategoryType(customerDetails.get(SystemConstants.ACCOUNT_CATEGORY_TYPE));
			...
			if (customerDetails.containsKey(SystemConstants.CUSTOMER_SEGMENT))
				...
				customerProfileTabDTO
						.setCustomerSegment(customerDetails.get(CommonServiceConstants.customerSegmentParam));
			...
			if (customerDetails.containsKey(SystemConstants.FREQ_USED_LOCATION))
				customerProfileTabDTO
						.setFrequentlyUsedLocation(customerDetails.get(SystemConstants.FREQ_USED_LOCATION));
			...
			if (customerDetails.containsKey(SystemConstants.MOBILE_APP_SUBSCRIPTION_STATUS))
				;
			customerProfileTabDTO.setMobileAppSubscriptionStatus(
					customerDetails.get(SystemConstants.MOBILE_APP_SUBSCRIPTION_STATUS));
			...
			if (customerDetails.containsKey(SystemConstants.ACTIVATION_DATE))
				customerProfileTabDTO
						.setActivationDate(format.parse(customerDetails.get(SystemConstants.ACTIVATION_DATE)));
	...
	public boolean addingWelcomePointsOnRegistration(String requestId, String timeStamp,
		...
		try {
			...
			if (((String) customerDetails.get("ACCOUNT_TYPE")).equalsIgnoreCase("POSTPAID")) {
				reResponseParameters.add(new ReResponseParameter("VOLUME_TAG",
						Cache.getConfigParameterMap().get("POSTPAID_WELCOME_POINTS").getParameterValue()));
				reResponseParameters.add(new ReResponseParameter("EXPIRY_DAYS",
						Cache.getConfigParameterMap().get("POSTPAID_WELCOME_POINTS_EXPIRY_DAYS").getParameterValue()));
				reResponseParameters.add(new ReResponseParameter("POINT_MULTIPLIER",
						Cache.getConfigParameterMap().get("POSTPAID_WELCOME_POINTS_MULTIPLIER").getParameterValue()));
			} else {
				reResponseParameters.add(new ReResponseParameter("VOLUME_TAG",
						Cache.getConfigParameterMap().get("PREPAID_WELCOME_POINTS").getParameterValue()));
				reResponseParameters.add(new ReResponseParameter("EXPIRY_DAYS",
						Cache.getConfigParameterMap().get("PREPAID_WELCOME_POINTS_EXPIRY_DAYS").getParameterValue()));
				reResponseParameters.add(new ReResponseParameter("POINT_MULTIPLIER",
						Cache.getConfigParameterMap().get("PREPAID_WELCOME_POINTS_MULTIPLIER").getParameterValue()));
			}
	...
	public HashMap<String, String> getBillingUserProfileDetails(String requestId, String mobileNumber) {
		...
		try {
			 ...
			 * "		\"request_timestamp\": \"02/20/2023 01:39:13\",\r\n" +
			 * "		\"response_timestamp\": \"02/20/2023 13:40:34\",\r\n" +
			 * "		\"action\": \"FetchServiceInfo\",\r\n" +
			 * "		\"source\": \"CRM\",\r\n" + "		\"username\": \"crmadmin\",\r\n"
			 * + "		\"userid\": \"1\",\r\n" + "		\"entity_id\": \"200\",\r\n" +
			 * "		\"result_code\": \"0\",\r\n" +
			 * "		\"result_desc\": \"Success\",\r\n" +
			 ...
			 * + "				\"recordcnt\": 1,\r\n" +
			 * "				\"service_info\": [{\r\n" +
			 * "					\"service_type_id\": \"72184\",\r\n" +
			 * "					\"account_id\": \"1000181591\",\r\n" +
			 * "					\"profile_id\": \"138431\",\r\n" +
			 * "					\"service_name\": \"GSM\",\r\n" +
			 * "					\"rate_plan_id\": \"\",\r\n" +
			 * "					\"rate_plan_name\": \"\",\r\n" +
			 * "					\"connection_type\": \"2\",\r\n" +
			 * "					\"service_id\": \"12687733838\",\r\n" +
			 * "					\"activation_date\": \"08/23/2022 13:17:31\",\r\n" +
			 * "					\"imsi_number\": \"344030000266622\",\r\n" +
			 * "					\"first_name\": \"SHERNELL \",\r\n" +
			 * "					\"last_name\": \"SERENITY  \",\r\n" +
			 * "					\"status\": \"1\",\r\n" +
			 * "					\"is_parent\": \"1\",\r\n" +
			 * "					\"parent_seq_id\": \"\",\r\n" +
			 * "					\"account_first_name\": \"SHERNELL BURTON  SERENITY COLBOURNE \",\r\n"
			 * + "					\"account_last_name\": \"SERENITY COLBOURNE \",\r\n" +
			 * "					\"icc_id\": \"8901030022041125427\",\r\n" +
			 * "					\"network_type\": \"2\",\r\n" +
			 * "					\"migrated_subscriber_id\": \"\",\r\n" +
			 * "					\"network_type_value\": \"GSM\",\r\n" +
			 * "					\"pin\": \"\",\r\n" +
			 * "					\"service_group_id\": \"\",\r\n" +
			 * "					\"tnc_date\": \"\",\r\n" +
			 * "					\"dir_display_name\": \"\",\r\n" +
			 * "					\"dir_address_opted_out\": \"\",\r\n" +
			 * "					\"is_optedInForDirectoryListing\": \"\",\r\n" +
			 * "					\"exchange_carrier\": \"\",\r\n" +
			 * "					\"complex_name\": \"\",\r\n" +
			 * "					\"billing_flag\": \"\",\r\n" +
			 * "					\"billing_flag_comment\": \"\",\r\n" +
			 * "					\"is_shared_account\": \"0\",\r\n" +
			 * "					\"profile_info\": [{\r\n" +
			 * "						\"profile_id\": \"138431\",\r\n" +
			 * "						\"profile_type\": \"1\",\r\n" +
			 * "						\"subscriber_category\": \"1\",\r\n" +
			 * "						\"first_name\": \"SHERNELL BURTON \",\r\n" +
			 * "						\"middle_name\": \"\",\r\n" +
			 * "						\"last_name\": \"SERENITY COLBOURNE \",\r\n" +
			 * "						\"contact_number_1\": \"\",\r\n" +
			 * "						\"contact_email_id\": \"\",\r\n" +
			 * "						\"pin\": \"\",\r\n" +
			 * "						\"ssn\": \"\",\r\n" +
			 * "						\"tin_ein\": \"\",\r\n" +
			 * "						\"customer_type\": \"3\",\r\n" +
			 * "						\"subscriber_sub_category\": \"22\",\r\n" +
			 * "						\"addresses\": [{\r\n" +
			 * "							\"subscriber\": \"138431\",\r\n" +
			 * "							\"subscriber_level\": \"0\",\r\n" +
			 * "							\"address_type\": \"1\",\r\n" +
			 * "							\"addr_line1\": \" ALL SAINTS \",\r\n" +
			 * "							\"addr_line2\": \"ALL SAINTS \",\r\n" +
			 * "							\"addr_line3\": \"ST.JOHN\\u0027S \",\r\n" +
			 * "							\"addr_line4\": \"ANTIGUA AND BARBUDA\",\r\n" +
			 * "							\"addr_line5\": \"\",\r\n" +
			 * "							\"addr_line6\": \"\",\r\n" +
			 * "							\"addr_line7\": \"\",\r\n" +
			 * "							\"addr_line8\": \"\",\r\n" +
			...
			// String
			// response="{\"BillingSystem\":{\"request_id\":\"45674352\",\"request_timestamp\":\"02/17/2023
			// 01:09:31\",\"response_timestamp\":\"02/17/2023
			// 13:10:55\",\"action\":\"FetchServiceInfo\",\"source\":\"CRM\",\"username\":\"crmadmin\",\"userid\":\"1\",\"entity_id\":\"200\",\"result_code\":\"0\",\"result_desc\":\"Success\",\"upfront_payment\":\"NA\",\"response\":{\"dataset\":{\"totalrecordcnt\":1,\"recordcnt\":1,\"service_info\":[{\"service_type_id\":\"11002664941\",\"account_id\":\"1234567890\",\"profile_id\":\"9743\",\"service_name\":\"GSM
			// 5G POSTPAID\",\"rate_plan_id\":\"2431\",\"rate_plan_name\":\"Postpaid 5G
			// Plan\",\"connection_type\":\"1\",\"service_id\":\"9743303036\",\"activation_date\":\"\",\"imsi_number\":\"344030000008815\",\"first_name\":\"\",\"last_name\":\"\",\"status\":\"1\",\"is_parent\":\"1\",\"parent_seq_id\":\"\",\"account_first_name\":\"Shared
			// Test\",\"account_last_name\":\"Test\",\"icc_id\":\"344030000008815\",\"network_type\":\"2\",\"migrated_subscriber_id\":\"\",\"network_type_value\":\"GSM\",\"pin\":\"\",\"service_group_id\":\"\",\"tnc_date\":\"\",\"dir_display_name\":\"\",\"dir_address_opted_out\":\"\",\"is_optedInForDirectoryListing\":\"\",\"exchange_carrier\":\"\",\"complex_name\":\"\",\"billing_flag\":\"\",\"billing_flag_comment\":\"\",\"is_shared_account\":\"1\",\"profile_info\":[{\"profile_id\":\"5001668906\",\"profile_type\":\"0\",\"subscriber_category\":\"1\",\"first_name\":\"Shared\",\"middle_name\":\"\",\"last_name\":\"Test\",\"contact_number_1\":\"8569124763\",\"contact_email_id\":\"afridshaikh5497@gmail.com\",\"pin\":\"\",\"ssn\":\"\",\"tin_ein\":\"\",\"customer_type\":\"Residential\",\"subscriber_sub_category\":\"3\",\"addresses\":[{\"subscriber\":\"5001668906\",\"subscriber_level\":\"0\",\"address_type\":\"1\",\"addr_line1\":\"Street1\",\"addr_line2\":\"Alao\",\"addr_line3\":\"Parish1\",\"addr_line4\":\"ALBANIA\",\"addr_line5\":\"\",\"addr_line6\":\"\",\"addr_line7\":\"\",\"addr_line8\":\"\",\"company_name\":\"\",\"branch\":\"\"}]}],\"profile_first_name\":\"Shared\",\"profile_last_name\":\"Test\"}]}}}}";
			...
			logger.info("TransactionId " + requestId + " Response " + response.toString()+"TotalTime:"+(System.currentTimeMillis() - l1));
			...
			if (billingResponse != null && billingResponse.getBillingSystem() != null
					&& billingResponse.getBillingSystem().getResult_code() != null
				...
				if (billingResponse.getBillingSystem().getResponse() != null
						&& billingResponse.getBillingSystem().getResponse().getDataset() != null
						&& billingResponse.getBillingSystem().getResponse().getDataset().getService_info() != null) {
	...
	public HashMap<String, String> setProfileInformation(Dataset dataSetInfo,
			HashMap<String, String> fetchProfileDetails) {
		List<Address> listAddress = null;
		String addresString = null;
		...
		if (dataSetInfo.getTotalrecordcnt() > 1) {
			for (ServiceInfo serviceInfo : dataSetInfo.getService_info()) {
				...
				fetchProfileDetails.put(SystemConstants.FIRSTNAME, serviceInfo.getFirst_name());
				fetchProfileDetails.put(SystemConstants.LASTNAME, serviceInfo.getLast_name());

				/*
				 * for (ProfileInfo profileInfo : serviceInfo.getProfile_info()) { listAddress =
				 * profileInfo.getAddresses(); }
				 * 
				 * listAddress.toString(); logger.info("ADDRESSES:" + listAddress.toString());
				 * fetchProfileDetails.put(SystemConstants.ADDRESSES, listAddress.toString());
				 */
			...
			for (ServiceInfo serviceInfo : dataSetInfo.getService_info()) {
				...
				fetchProfileDetails.put(SystemConstants.MSISDN, msisdnData);
				fetchProfileDetails.put(SystemConstants.ACCOUNT_NUM, serviceInfo.getAccount_id());
				fetchProfileDetails.put(SystemConstants.FNAME, serviceInfo.getFirst_name());
				fetchProfileDetails.put(SystemConstants.LNAME, serviceInfo.getLast_name());
			}
		}
	...
	public static void main(String arg[]) throws ParseException {
		/*
		 * DateFormat format = new SimpleDateFormat("MM/dd/yyyy"); String
		 * date="11/14/2002"; System.out.println(format.parse(date));
		 * 
		 * Date date = new Date(); SimpleDateFormat formatter = new
		 * SimpleDateFormat("MM/dd/yyyy hh:mm:ss"); String strDate=
		 * formatter.format(date); System.out.println(strDate);
		 */
