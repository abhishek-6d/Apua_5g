...
public class PointExpiryBO implements BusinessLogics {
			...
			if (request != null) {
				...
				if (dataSet != null) {
					...
					for (Parameters p : paramlist) {
						if (p.getName().equalsIgnoreCase("MSISDN"))
							msisdn = p.getValue();

						if (p.getName().equalsIgnoreCase("LOYALTY_ID"))
							loyaltyId = p.getValue();

						if (p.getName().equalsIgnoreCase("POINTS")) {
							points = p.getValue();
						}
				...
				if (msisdn != null) {
					msisdn = commonUtil.discardCountryCodeIfExists(request.getMsisdn());
				}

				logger.info("TransactionId " + requestId + " msisdn ::" + msisdn + " LoyaltyId::" + loyaltyId
						+ " Points::" + points);
				...
				if (loyaltyProfileTabDTO != null && Double.parseDouble(points) > 0) {
					if (loyaltyProfileTabDTO.getRewardPoints() >= Double.parseDouble(points)) {
						...
						if (al != null && (!al.get(0).equalsIgnoreCase("0") || !al.get(1).equalsIgnoreCase("0"))) {
							...
							if (redeemPointsDAO.updateLoyaltyProfileRedeem(loyaltyProfileTabDTO, Double.valueOf(points),
									Double.valueOf(tierPoints), Double.valueOf(bonuspoints))) {
								...
								reResponseDataSet.setParameters(paramlist);
								...
								loyaltyTransactionTabDTO.setCurRewardPoints(
										loyaltyProfileTabDTO.getRewardPoints() - Double.valueOf(points));
								...
								loyaltyTransactionTabDTO.setStatusDesc(Cache.getConfigParameterMap()
										.get("POINT_EXPIRED_TXN_STATUS_DESC").getParameterValue());
					...
					logger.info("TransactionId " + requestId + " No such user with msisdn ::" + msisdn
							+ "or loyalty id::" + loyaltyId);
					response = new REResponseHeader();
					response.setRequestId(requestId);
					response.setKeyWord(request.getKeyWord());
					response.setMsisdn(msisdn);
	...
	public void notifyRedemption(String msisdn, String points, String transactionId) {
		...
		DateFormat dateFormat = new SimpleDateFormat(
				Cache.getConfigParameterMap().get("NOTIFICATION_DATE_FORMAT").getParameterValue());
		Date date = new Date();
		String keyWord=null;
		try {
			...
			commonUtil.generateNotifyRequest(transactionId,keyWord, msisdn, notificationMap);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public CDRInformationDTO buildingCdrDtoAndWritingCdr(REResponseHeader response, String msisdn, String loyaltyId,
			String points, String transactionId) {
	...
	public Boolean captureTransactionPointExpiry(LoyaltyTransactionTabDTO loyaltyTransactionTabDTO) {
		...
		try {
			...
			logger.info("TransactionId " + loyaltyTransactionTabDTO.getReqTransactionID()
					+ " PointExpiryTransaction caputred status " + isTransactionCapturedStatus);
		} catch (Exception e) {
