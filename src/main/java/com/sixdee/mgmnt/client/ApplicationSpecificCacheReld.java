...
public class ApplicationSpecificCacheReld {
	...
	public ApplicationSpecificCacheReld(){
		//
	}
	...
	public static void main(String arg[])throws Exception{
		...
		if (arg.length == 0 || arg.length<4) {
			System.err.println("Usage: java -jar MgmntClient.jar ApplnSpecificCacheReload IpAddress Port LoginName LoginPassword");
			return;
		}
		...
		System.out.println("IP Address of the machine :- "+address);
		...
		String reqParam=arg[4].trim();

		
		ApplicationSpecificCacheReld ad=new ApplicationSpecificCacheReld();
		ad.createConnection(address,port);
		ad.runMgmntProcess(loginName,loginPwd,reqParam);
	...
	public String getAllCommandString(String requestParam) {

		String allCommandString = "<REQ><CMD>ApplnCacheRld</CMD><PARAMETERS>"
			+ "<MGMNT_REQUEST>"+ requestParam +"</MGMNT_REQUEST></PARAMETERS></REQ>";

		return allCommandString;
	}// end method getallCommand

	public void runMgmntProcess(String logName, String logPwd,String requestParam) {
		try {
			...
			if(!recievedInfo.contains("FAILURE")) {
				String xmlData = getAllCommandString(requestParam);
