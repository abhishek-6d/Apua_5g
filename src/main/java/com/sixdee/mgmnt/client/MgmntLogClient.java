...
public class MgmntLogClient {
	...
	public static void main(String arg[])throws Exception{
		...
		if (arg.length == 0 || arg.length<5) {
			System.err.println("Usage: java -jar MgmntClient.jar MgmntLogClient IpAddress port LoginName LoginPassword LogLevel(FATAL/ERROR/WARN/INFO/DEBUG)... ");
			return;
		}
		...
		String logLevel=arg[4].trim();//path of file to be read
		
		//LoadTestAllDataIPL ltd=new LoadTestAllDataIPL(path,logPath);
		MgmntLogClient ad=new MgmntLogClient();
		ad.createConnection(address,port);
		ad.runMgmntProcess(logLevel,logName,logPwd);
		//SenderThread ltd=new SenderThread(address,port,path,logPath);
	}//end main method
	...
	public String getAllCommandString(String logLevel) {

		String allCommandString = "<REQ><CMD>ModifyLogLevel</CMD><PARAMETERS><MGMNT_REQUEST>"
			+ logLevel + "</MGMNT_REQUEST></PARAMETERS></REQ>";

		return allCommandString;
	}// end method getallCommand

	public void runMgmntProcess(String LogLevel, String logName, String logPwd) {
		try {
			...
			if(!recievedInfo.contains("FAILURE")) {
				String xmlData = getAllCommandString(LogLevel);
